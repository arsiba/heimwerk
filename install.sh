...
print_header() {
    ...
    echo "╔══════════════════════════════════════════╗"
    echo "║                                          ║"
    echo "║      ${STAR} HEIMWERK INSTALLATION ${STAR}       ║"
    echo "║                                          ║"
    echo "╚══════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
read -rp "Enter Admin Username [admin]: " ADMIN_USER < /dev/tty
ADMIN_USER=${ADMIN_USER:-admin}

read -rp "Enter Admin Email [admin@example.com]: " ADMIN_EMAIL < /dev/tty
ADMIN_EMAIL=${ADMIN_EMAIL:-admin@example.com}

while true; do
    read -rsp "Enter Admin Password (min. 8 chars): " ADMIN_PASS < /dev/tty
    echo ""
    validate_password "$ADMIN_PASS" && break
...
until docker compose -f "${COMPOSE_FILE}" exec -T db pg_isready -U heimwerk_admin &>/dev/null; do
    sleep 2
done
...
print_step "6/7" "Database & Static Files"
...
docker compose -f "${COMPOSE_FILE}" exec -T \
    ...
    heimwerk python manage.py createsuperuser --no-input || true
